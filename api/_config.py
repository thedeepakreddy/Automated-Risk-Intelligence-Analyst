...
# Requests per minute to pace classification at. The free tier for
# gemini-2.5-flash allows roughly 10/min, and a cycle classifying 20 headlines
# back-to-back will exhaust that in seconds -- every call past the limit gets a
# 429 and silently lands in degraded keyword mode. Set 0 to disable pacing.
def gemini_max_rpm() -> int:
    return _env_int("GEMINI_MAX_RPM", 10)


# Retries on a throttled (429) or transient (5xx) response, with exponential
# backoff, before an article gives up and falls back to keyword mode.
def gemini_max_retries() -> int:
    return _env_int("GEMINI_MAX_RETRIES", 2)


# After this many consecutive quota failures the cycle stops calling Gemini
# altogether and classifies the remainder by keyword. Once the daily cap is
# spent, retrying every remaining article just makes the cycle slow as well as
# degraded.
def gemini_breaker_threshold() -> int:
    return _env_int("GEMINI_BREAKER_THRESHOLD", 3)
