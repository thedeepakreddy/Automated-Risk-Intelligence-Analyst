...
from api._classifier import build_client, classify_article, quota_breaker_open, reset_quota_breaker
...
def run_ingestion_cycle(limit: Optional[int] = None) -> Dict:
    ...
    try:
        ...
        if client is not None and quota_breaker_open():
            log.error("[INGESTION] Gemini quota breaker tripped this cycle - see the CLASSIFIER "
                      "log lines above for the provider error")
