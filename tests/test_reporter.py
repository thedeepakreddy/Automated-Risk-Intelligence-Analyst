...
def test_context_is_derived_from_the_articles():
    context = build_context(ARTICLES, SCORES)
...
def test_prompt_carries_the_real_headlines_and_score():
    prompt = build_prompt(build_context(ARTICLES, SCORES), ARTICLES)
...
def test_fallback_report_cites_this_day_s_specifics():
    report = render_fallback_report(build_context(ARTICLES, SCORES))
...
def test_fallback_differs_as_the_data_differs():
    ...
    first = render_fallback_report(build_context(ARTICLES, SCORES))
    second = render_fallback_report(build_context(other, [{"timestamp": "2026-07-29T11:30:00+00:00", "score": 22.0}]))
